// ==UserScript==
// @name         ChatGPT Prompt中文提示
// @namespace    http://tampermonkey.net/
// @version      0.0.1
// @description  ChatGPT Prompt中文提示
// @author       AndyZ
        ...
        if (checkDom()) {
            ...
            const Observer = new MutationObserver(function (mutations) {
                for (const mutation of mutations) {
                    ...
                    if (mutation.target.getAttribute("class")?.includes("prompt-list")) {
                        const listWrapperDom = document.querySelector(
                            ".prompt-list>.prompt-list__wrapper"
                        );
                        if (listWrapperDom) {
                            // 事件代理
                            listWrapperDom.onclick = function (e) {
                                const target = e.target.closest("div.prompt-list-item");
                                if (target) {
                                    const prompt = target.getAttribute("data-prompt");
                                    const textareaDom = document.querySelector("textarea");

                                    textareaDom.value = decodeURI(prompt)?.replace(/\n$/, "");
                                    textareaDom.focus();
                                }
                            };
                        }
...
function checkDom() {
    const inputBoxDom = document.getElementsByClassName(
...
function promptTip() {
    ...
    if (inputBoxDom.length > 0) {
        let listDom = document.querySelector(".prompt-list");
        if (!listDom) {
            const ele = document.createElement("div");
            ele.className =
                "prompt-list border border-black/10 bg-white dark:border-gray-900/50 dark:text-white dark:bg-gray-700 rounded-md shadow-[0_0_10px_rgba(0,0,0,0.10)] dark:shadow-[0_0_15px_rgba(0,0,0,0.10)]";

            listDom = inputBoxDom[0].appendChild(ele);

            const textareaDom = document.querySelector("textarea");

            textareaDom.oninput = textareaDomInputHandler;
            function textareaDomInputHandler() {
            ...
            textareaDom.onkeydown = textareaDomKeydownHandler;
            function textareaDomKeydownHandler(e) {
                // 回车
                if (e.keyCode === 13) {
                    hideListDom();
                }
            ...
            //   textareaDom.onfocus = showListDom;
            //   textareaDom.onblur = hideListDom;

            function showListDom() {
                listDom.style.borderWidth = "1px";
                listDom.style.height = "300px";
                renderList();
            }
            function hideListDom() {
                listDom.style.borderWidth = "0px";
                listDom.style.height = "0px";
            }
            function renderList() {
                // TODO 渲染列表每一项
                const promptItemHtml = promptList
                .map(
                    (e) =>
                    `<div class="prompt-list-item" data-prompt="${encodeURI(
                        e.prompt
                    )}">
                <span class="prompt-list-item__preffix"></span>
                <span class="prompt-list-item__content">
                    <span class="title">${e.act}</span>
                    <span class="desc"></span>
                </span>
                <span class="prompt-list-item__suffix"></span>
            </div>`
          )
        .join("");
          listDom.innerHTML = `
            <div class="prompt-list__wrapper">
                ${promptItemHtml}
            </div>
        `;
      }
